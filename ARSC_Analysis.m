...
t=MaxWave;
correction=0.0000 %Max Wavenumber
...
[fitresultII, gofy]= createSpline(XX, A);         %Raw Data SplineFit
   ...
   for d=winstart:winend                              %Working Arrays loading
    e=winend+1-d; 
    XXX(e)=d;
    AA(e)=fitresultII(d);                %Remove Background from Lorentzian and store in AA
                            %Subtract Lorentzian from Raw store in CC
   end
    [fitresultII, gofy]= createSpline(XXX, AA);         %Raw Data SplineFit

   % end
   for d=winstart:winend                              %Working Arrays loading
    e=winend+1-d; 
    XXXX(e)=d;
    AAA(e)=fitresultII(d);                %Remove Background from Lorentzian and store in AA
                            %Subtract Lorentzian from Raw store in CC
    end
[fitresult,gof]=createFit(XXXX, AAA, aproxX);
...
opts.Lower=[pplow 1000 1000 0.1];
%opts.StartPoint = [aproxX 4 200 4];
opts.StartPoint = [aproxX 2000 2000 3]; %0.97
opts.Upper = [pphigh Inf Inf 10];
%            [pp   A  back  width]
